...
import { API_GET_ALL_COURSE_HIGHLIGHTS, API_GET_HIGHLIGHTS } from "@constants";
...
export const HighlightsView = () => {
  ...
  useEffect(() => {
    const loadUserHighlights = async () => {
      try {
        const res = await fetch(API_GET_HIGHLIGHTS, { headers: authHeaders() });
        const result = await res.json();
        if (res.ok && Array.isArray(result.data)) {
          setUserHighlights(result.data);
        }
      } catch {
        // ignore user highlights loading errors
      }
    };
    loadUserHighlights();
  }, []);
